...
ALLOWED_SCAN_EXTENSIONS = {'nii', 'gz'}
ALLOWED_VIDEO_EXTENSIONS = {'mp4', 'webm', 'mov', 'avi'}


def get_user_id():
    """Get or create an anonymous user ID from cookie."""
...
def new_case():
    if request.method == 'POST':
        ...
        if 'scan_file' in request.files:
            ...
            if scan_file.filename and allowed_scan(scan_file.filename):
                filename = secure_filename(f"case_{case_id}_{scan_file.filename}")
                filepath = os.path.join(current_app.config['UPLOAD_FOLDER'], filename)
                scan_file.save(filepath)
...
def delete_case(case_id):
    ...
    if case:
        # Clean up files
        for fname in [case.get('nifti_filename'), case.get('glb_filename'), case.get('video_filename')]:
            if fname:
...
def upload_scan(case_id):
    ...
    scan_file = request.files['scan_file']
    if not scan_file.filename or not allowed_scan(scan_file.filename):
...
def upload_file():
    ...
    if file and allowed_scan(file.filename):
